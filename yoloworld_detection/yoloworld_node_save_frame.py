...
class DetectionNode(Node):
    ...
    def detect_callback(self, data):
        ...
        if key & 0xFF == ord('c'):  # 'C' 키 입력
            self.capture_and_send_frame(cv_image)
       


    def capture_and_send_frame(self, frame):
        """현재 프레임을 저장하고 서버에 전송"""
        # 저장 파일 경로 설정
        ...
        try:
            response = requests.post(
                self.server_url,
                files={"file": (filename, image_data, "image/jpeg")} # file : 서버로 전송될 파일 이름, image_data: 파일의 실제 데이터(io.BytesIO(buffer)로 생성된 JPEG 이미지 데이터), "image/jpeg": 이 값은 파일의 형식을 나타내며, 이미지 파일에서는 "image/jpeg"를 사용, MIME 타입은 서버가 파일의 내용을 처리하는 방식을 결정하는 데 사용
            )
            if response.status_code == 200:
                self.get_logger().info(f"Frame sent to server successfully: {response.json()}")
            else:
                self.get_logger().error(f"Failed to send frame to server. Status code: {response.status_code}")
        except Exception as e:
            self.get_logger().error(f"Error while sending frame to server: {e}")
